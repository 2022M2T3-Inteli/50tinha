...
app.post('/profissionais/atualizar', urlencodedParser, (req, res) => {
	...
	sql = "UPDATE profissionais SET (name, hours, field, role, type) VALUES ('" + req.body.name + "', " + req.body.hours + ",'"+ req.body.field +"','" + req.body.role +"','" + req.body.type +"')";
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
		    throw err;
...
app.all('/profissionais/deletar', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); // Isso é importante para evitar o erro de CORS
...
//insere um novo registro (C do CRUD)
app.post('/projetos/adicionar', urlencodedParser, (req, res) => {
	...
	sql = "INSERT INTO projetos (name, id, area, numberFunc, duration) VALUES ('" + req.body.name + "', " + req.body.id + ", '" + req.body.area + "', '" + req.body.numberFunc + "', " + req.body.duration + ")";
	var db = new sqlite3.Database(DBPATH); //Abre o banco
	console.log(sql);
	db.run (sql, [], err => {
		if (err) {
...
// Atualiza um registro (é o U do CRUD - Update)
app.post('/projetos/atualizar', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); // Isso é importante para evitar o erro de CORS

	sql = "UPDATE projetos SET (name, id, area, numberFunc, duration) VALUES ('" + req.body.name + "', " + req.body.id + ", '" + req.body.area + "', '" + req.body.numberFunc + "', " + req.body.duration + ")";
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
		    throw err;
...
app.all('/projetos/deletar', urlencodedParser, (req, res) => {
    res.statusCode = 200;
    res.setHeader('Access-Control-Allow-Origin', '*'); // Isso é importante para evitar o erro de CORS
    sql = "DELETE FROM projetos WHERE name = " + "'" + req.body.name + "'";
    var db = new sqlite3.Database(DBPATH); // Abre o banco
    db.run(sql, [],  err => {
        if (err) {
            throw err;
